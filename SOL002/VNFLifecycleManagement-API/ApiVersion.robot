...
POST API Version
    ...
    Post    ${apiRoot}/${apiName}/${apiVersion}
...
GET API Version
    Set Headers    {"Accept":"${ACCEPT}"} 
    Run Keyword If    ${AUTH_USAGE} == 1    Set Headers    {"Authorization":"${AUTHORIZATION}"}
    Get    ${apiRoot}/${apiName}/${apiVersion}
    ${outputResponse}=    Output    response
	Set Global Variable    @{response}    ${outputResponse} 
...
PUT API Version
    Set Headers    {"Accept":"${ACCEPT}"} 
    Run Keyword If    ${AUTH_USAGE} == 1    Set Headers    {"Authorization":"${AUTHORIZATION}"}
    Put    ${apiRoot}/${apiName}/${apiVersion}
    ${outputResponse}=    Output    response
	Set Global Variable    @{response}    ${outputResponse} 
...
PATCH API Version
    Set Headers    {"Accept":"${ACCEPT}"} 
    Run Keyword If    ${AUTH_USAGE} == 1    Set Headers    {"Authorization":"${AUTHORIZATION}"}
    Patch    ${apiRoot}/${apiName}/${apiVersion}
    ${outputResponse}=    Output    response
	Set Global Variable    @{response}    ${outputResponse} 
...
DELETE API Version
    Set Headers    {"Accept":"${ACCEPT}"} 
    Run Keyword If    ${AUTH_USAGE} == 1    Set Headers    {"Authorization":"${AUTHORIZATION}"}
    Delete    ${apiRoot}/${apiName}/${apiVersion}
    ${outputResponse}=    Output    response
	Set Global Variable    @{response}    ${outputResponse} 